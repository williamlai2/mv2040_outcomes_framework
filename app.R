...
circ_inds1 <- towards_target %>%
    ...
    left_join(data_format, by = "id") %>% 
    select(id, baseline, progress, target, theme, desired, toward_pct, measure, category, value_format = value_format.x) %>% 
...
sidebar <- dashboardSidebar(
    ...
    sidebarMenu(
        ...
        menuItem("Summaries",
                 ...
                 menuSubItem("Progress by influence", tabName = "influence")
        ),
...
body <- dashboardBody(
    ...
    tabItems(
        ...
        tabItem(tabName = "notes",
                ...
                box(title = "Notes:", "This is a work in progress. Contact research@mvcc.vic.gov.au if you have any queries.", width = 12)
                )
        )
    )
)
